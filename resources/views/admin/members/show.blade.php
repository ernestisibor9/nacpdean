...
@section('admin')
    ...
    {{-- Shared ID card styles --}}
    @include('partials.member-id-card-styles')

    @php
        /*
    |--------------------------------------------------------------------------
    | ID CARD DATA — only for approved members
    |--------------------------------------------------------------------------
    */

        $idCardMembership = null;
        $idCardCategory = null;
        $idCardMembershipCard = null;
        $idCardIsActive = false;
        $idCardDebitNotPaid = false;
        $idCardMemberName = 'Member';

        if ($application->status === 'approved') {
            $idCardMembership = \App\Models\Membership::with('membershipCategory')
                ->where('user_id', $application->user_id)
                ->latest('id')
                ->first();

            $idCardCategory = $idCardMembership?->membershipCategory ?? $application->membershipCategory;

            if ($idCardMembership) {
                $idCardMembershipCard = \App\Models\MembershipCard::where('membership_id', $idCardMembership->id)
                    ->where('status', 'active')
                    ->latest('id')
                    ->first();
            }

            $idCardDebit = \App\Models\Transaction::where('user_id', $application->user_id)
                ->where('type', 'debit')
                ->whereNull('payment_item_id')
                ->latest('id')
                ->first();

            $idCardDebitNotPaid = $idCardDebit && $idCardDebit->status === 'not paid';

            if ($idCardMembership && $idCardMembership->expires_at) {
                $idCardIsActive =
                    $idCardMembership->status === 'active' &&
                    now()->lessThanOrEqualTo(\Carbon\Carbon::parse($idCardMembership->expires_at)->endOfDay());
            } elseif ($idCardMembership) {
                $idCardIsActive = $idCardMembership->status === 'active';
            }

            $nameParts = array_filter([$application->first_name, $application->middle_name, $application->surname]);

            if (!empty($nameParts)) {
                $idCardMemberName = implode(' ', $nameParts);
            }
        }
    @endphp
    ...
    ========================================================== --}}
        ...
        ====================================================== --}}
            <div class="col-lg-8">
                ...
                {{-- ============================================================
                 ID CARD PREVIEW (only for approved members)
            ============================================================ --}}
                @if ($application->status === 'approved' && $idCardMembershipCard)
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            ">
                                ...
                                @include('partials.member-id-card', [
                                    'profile' => $application,
                                    'membership' => $idCardMembership,
                                    'membershipCategory' => $idCardCategory,
                                    'membershipCard' => $idCardMembershipCard,
                                    'membershipIsActive' => $idCardIsActive,
                                    'membershipDebitNotPaid' => $idCardDebitNotPaid,
                                    'memberName' => $idCardMemberName,
                                ])
                            ...
                            ">
                                ...
                                <div>
                                    <strong
                                        style="display:block; color:#111827; font-size:14px; font-weight:800; margin-bottom:4px;">
                                        Membership ID Card
                                    </strong>
                                    <span style="display:block; color:#6b7280; font-size:12px; line-height:1.5;">
                                        Keep this membership identification card available for official use.
                                    </span>
                                </div>

                                @if (!$idCardDebitNotPaid && $idCardIsActive)
                                    <a href="{{ route('admin.member.id-card', $application->user_id) }}"
                                        class="btn btn-outline">
                                        <i class="fas fa-expand"></i>
                                        View ID Card
                                    </a>
                                @elseif (!$idCardDebitNotPaid && !$idCardIsActive)
                                    <span class="badge badge-danger">
                                        <i class="fas fa-exclamation-circle"></i>
                                        Membership Expired
                                    </span>
                                @elseif ($idCardDebitNotPaid)
                                    <span class="badge badge-danger">
                                        <i class="fas fa-exclamation-circle"></i>
                                        Payment Required
                                    </span>
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="row g-4">
                            ...
                            <div class="col-md-6">
                                <div class="field">
                                    <div class="field-label"><i class="fas fa-id-card"></i> Business Registration Number
                                    </div>
        ...
        ====================================================== --}}
            <div class="col-lg-4">
                <div class="sidebar-sticky">
                    ...
                    @if (
                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                ...
                                {{-- PRIMARY: RENEW NOW (delegates to frontend flow) --}}
                                <a href="{{ route('admin.member.renew', $application->user_id) }}"
                                ...
                                <p
                                    style="color:#6b7280; font-size:11px; margin-top:10px; margin-bottom:0; line-height:1.6;">
                                    This opens the same secure renewal payment page
                                    the member sees — with the renewal fee pre-filled.
                                    Complete the Paystack checkout to renew the membership.
                                </p>
                    ...
                    @if ($application->status === 'approved')
                        <a href="{{ route('admin.member.edit-profile', $application->user_id) }}"
